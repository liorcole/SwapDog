...
final class FirestoreDogRepository: DogRepositoryProtocol {
    ...
    func addDog(_ dog: Dog, ownerID: String) async throws {
        logger.info("addDog id=\(dog.id) ownerID=\(ownerID)")
        do {
            try await db
            ...
            logger.error("addDog failed: \(error.localizedDescription)")
            throw error
        } catch {
            logger.error("addDog unexpected error: \(error.localizedDescription)")
            throw SwapDogError.networkError
        }
    }

    func getDogs(ownerID: String) async throws -> [Dog] {
        logger.info("getDogs ownerID=\(ownerID)")
        do {
            ...
            logger.error("getDogs failed: \(error.localizedDescription)")
            throw error
        } catch {
            logger.error("getDogs unexpected error: \(error.localizedDescription)")
            throw SwapDogError.networkError
        }
    }

    func updateDog(_ dog: Dog) async throws {
        logger.info("updateDog id=\(dog.id) ownerID=\(dog.ownerID)")
        do {
            try await db
            ...
            logger.error("updateDog failed: \(error.localizedDescription)")
            throw error
        } catch {
            logger.error("updateDog unexpected error: \(error.localizedDescription)")
            throw SwapDogError.networkError
        }
    }

    func deleteDog(id: String, ownerID: String) async throws {
        logger.info("deleteDog id=\(id) ownerID=\(ownerID)")
        do {
            try await db
                .collection(FirestorePaths.userDogs(ownerID: ownerID))
                .document(id)
            ...
            logger.error("deleteDog failed: \(error.localizedDescription)")
            throw error
        } catch {
            logger.error("deleteDog unexpected error: \(error.localizedDescription)")
            throw SwapDogError.networkError
        }
    }

    func uploadDogPhoto(data: Data, dogID: String) async throws -> String {
        logger.info("uploadDogPhoto dogID=\(dogID) bytes=\(data.count)")
        do {
            ...
            logger.error("uploadDogPhoto failed: \(error.localizedDescription)")
            throw error
        } catch {
            logger.error("uploadDogPhoto unexpected error: \(error.localizedDescription)")
